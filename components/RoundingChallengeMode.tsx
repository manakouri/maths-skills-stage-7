...
import { CHALLENGE_DURATION_SECONDS, ROUNDING_HIGH_SCORE_STORAGE_KEY } from '../constants';
import { generateQuestion } from '../utils/questionGenerator';
import { QuestionCard } from './QuestionCard';
import { Button } from './Button';
import { TimerIcon, TrophyIcon } from './icons';

interface RoundingChallengeModeProps {
  onBack: () => void;
  colorTheme: ColorTheme;
}

const ROUNDING_MODES: GameMode[] = [
...
const generateRandomRoundingQuestion = (): Question => {
  const randomMode = ROUNDING_MODES[Math.floor(Math.random() * ROUNDING_MODES.length)];
  return generateQuestion(randomMode);
};

const RoundingChallengeMode: React.FC<RoundingChallengeModeProps> = ({ onBack, colorTheme }) => {
  ...
  useEffect(() => {
    const storedHighScore = localStorage.getItem(ROUNDING_HIGH_SCORE_STORAGE_KEY);
  ...
  const startGame = useCallback(() => {
    ...
    setQuestion(generateRandomRoundingQuestion());
  ...
  useEffect(() => {
      if (gameState === 'results') {
          if (score > highScore) {
              ...
              localStorage.setItem(ROUNDING_HIGH_SCORE_STORAGE_KEY, score.toString());
          }
      }
  ...
  if (gameState === 'intro') {
    return (
      <div className="bg-white text-slate-800 p-8 rounded-xl shadow-xl border border-slate-200 text-center animate-fade-in">
        <h2 className="text-3xl font-bold text-slate-900 mb-4">Rounding Challenge</h2>
        <p className="text-lg text-slate-600 mb-2">Answer as many rounding questions as you can in {CHALLENGE_DURATION_SECONDS} seconds.</p>
...
export default RoundingChallengeMode;