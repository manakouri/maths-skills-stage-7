...
import { GameMode, Question, AnswerStatus, FDPValue, ColorTheme } from '../types';
import { ORDERING_CHALLENGE_DURATION_SECONDS, ORDERING_HIGH_SCORE_STORAGE_KEY } from '../constants';
import { generateQuestion } from '../utils/questionGenerator';
import { OrderingQuestionCard } from './OrderingQuestionCard';
import { Button } from './Button';
import { TimerIcon, TrophyIcon } from './icons';

interface OrderingChallengeModeProps {
  onBack: () => void;
  colorTheme: ColorTheme;
}

const OrderingChallengeMode: React.FC<OrderingChallengeModeProps> = ({ onBack, colorTheme }) => {
  const [gameState, setGameState] = useState<'intro' | 'playing' | 'results'>('intro');
  const [question, setQuestion] = useState<Question | null>(null);
  const [score, setScore] = useState(0);
  const [timeLeft, setTimeLeft] = useState(ORDERING_CHALLENGE_DURATION_SECONDS);
  const [highScore, setHighScore] = useState(0);
  const [isNewHighScore, setIsNewHighScore] = useState(false);
  const [answerStatus, setAnswerStatus] = useState<AnswerStatus>(AnswerStatus.UNANSWERED);
  const [correctAnswerForDisplay, setCorrectAnswerForDisplay] = useState<FDPValue[] | null>(null);
  const timerRef = useRef<number | null>(null);
  const isPausedRef = useRef(false);

  useEffect(() => {
    const storedHighScore = localStorage.getItem(ORDERING_HIGH_SCORE_STORAGE_KEY);
  ...
  const startGame = useCallback(() => {
    setScore(0);
    setTimeLeft(ORDERING_CHALLENGE_DURATION_SECONDS);
    setIsNewHighScore(false);
    setQuestion(generateQuestion(GameMode.ORDERING_FDP));
  ...
  useEffect(() => {
      if (gameState === 'results' && score > highScore) {
          ...
          localStorage.setItem(ORDERING_HIGH_SCORE_STORAGE_KEY, score.toString());
      }
  ...
  const handleAnswerSubmit = (userAnswer: FDPValue[]) => {
    if (answerStatus !== AnswerStatus.UNANSWERED || !question) return;

    const correctAnswer = question.answer as FDPValue[];
    const isCorrect = userAnswer.length === correctAnswer.length && userAnswer.every((item, index) => item.id === correctAnswer[index].id);
  ...
  if (gameState === 'intro') {
    return (
      <div className="bg-white text-slate-800 p-8 rounded-xl shadow-xl border border-slate-200 text-center animate-fade-in">
        <h2 className="text-3xl font-bold text-slate-900 mb-4">Ordering Challenge</h2>
        <p className="text-lg text-slate-600 mb-2">Order the values as quickly as you can in 3 minutes.</p>
...
export default OrderingChallengeMode;